...
class ParsingAlgos:
    ...
    def _collect_residues_all(dssr_data, feature):
        ...
        if feature in ("hairpins", "bulges", "junctions", "stacks", "nonstack"):
            ...
            if feature == "nonstack" and isinstance(entries, dict):
                entries = (
                    [entries]
                    if entries.get("num_nts", 0) > 0 or entries.get("nts_long")
                    else []
                )
    ...
    def _build_residue_sel_from_dssr(dssr_data, feature, index):
        ...
        if feature == "nonstack" and isinstance(feature_list, dict):
            feature_list = (
                [feature_list]
                if feature_list.get("num_nts", 0) > 0 or feature_list.get("nts_long")
                else []
            )
        if (
            feature_list is None
            or not isinstance(feature_list, list)
            or len(feature_list) == 0
        ):
    ...
    def dssr_select(
        ...
        try:
            ...
            if feature == "nonstack" and isinstance(feature_list, dict):
                feature_list = (
                    [feature_list]
                    if feature_list.get("num_nts", 0) > 0
